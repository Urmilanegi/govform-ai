...
export async function GET(request: NextRequest) {
  ...
  const stream  = new ReadableStream({
    start(controller) {
      ...
      // Kill any previously running upsc-fill or leftover chromium using this profile
      try {
        const { execSync } = require('child_process');
        execSync("pkill -9 -f 'upsc-fill' 2>/dev/null || true");
        execSync("pkill -9 -f 'upsc-profile' 2>/dev/null || true");
        // Remove stale lock files
        const profileDir = getGovformProfileDir('upsc-profile');
        ['SingletonLock','SingletonCookie','SingletonSocket'].forEach((f: string) => {
          try { require('fs').unlinkSync(profileDir + '/' + f); } catch {}
        });
      } catch {}

      send({ type: 'start', message: '🚀 UPSC automation shuru ho rahi hai...' });
      ...
      const scriptPath = scriptsDir + path.sep + 'upsc-fill.cjs';
