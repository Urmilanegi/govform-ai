...
// POST — submit OTP, captcha, or any user input
...
export async function GET(request: NextRequest) {
  ...
  const student = {
    ...
    EXAM_NAME:    p.get('examName') || 'uptet',
  ...
  const stream  = new ReadableStream({
    start(controller) {
      ...
      send({ type: 'start', message: '🚀 UPTET automation shuru ho rahi hai...' });
      ...
      const scriptPath = scriptsDir + path.sep + 'uptet-fill.cjs';
      const child = spawn(process.execPath, [scriptPath], {
        env: { ...process.env, ...student },
