...
(async () => {
  ...
  const vals = await page.evaluate(() => [...document.querySelectorAll('app-dropdown-new')].map(d => ({
    label: (d.getAttribute('label') || '').slice(0, 40),
    selectType: (d.querySelector('.select-type')?.textContent || 'NONE').trim().slice(0, 40),
  })));
